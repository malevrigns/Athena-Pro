...
| 区域 | 文件数 | 行数 | 说明 |
|---|---:|---:|---|
| `backend_py` (`src/athena/`) | 106 | 11032 | 可运行后端源码(FastAPI + 多 Agent runtime + Research OS 层 + agent runtime)。 |
| `frontend_src` (`web/src/`) | 55 | 12956 | Vue 3 + Vite + Pinia + Element Plus 前端源码(`.ts` / `.vue` / `.css`)。 |
| `tests` (`tests/`) | 20 | 4047 | 后端 pytest 测试(含 `tests/research/` Research OS 套件)。 |
...
结果:`237 passed`;后端编译通过;前端 `vite build` 通过。

...
| 区域 | 文件数 | 行数 | 说明 |
|---|---:|---:|---|
| `backend_py` (`src/athena/`) | 112 | 12043 | 可运行后端源码(FastAPI + 多 Agent runtime + Research OS 层 + agent runtime)。 |
| `frontend_src` (`web/src/`) | 55 | 12956 | Vue 3 + Vite + Pinia + Element Plus 前端源码(`.ts` / `.vue` / `.css`)。 |
| `tests` (`tests/`) | 23 | 4386 | 后端 pytest 测试(含 `tests/research/` Research OS 套件)。 |
...
结果:`248 passed`;后端编译通过;前端 `vite build` 通过。
